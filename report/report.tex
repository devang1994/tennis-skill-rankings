...
Each of the \emph{Win i} events is parameterized by a CPD that depends on the skills of the players.
In the Bradley-Terry model, we might have:
\[
...
where
\[
A\left(\mathrm{Skills}\right) \tridefeq \exp \left( \sum_i \textrm{Player i Off. Skill} \right)
%\quad, \quad \quad
\]%
\[
B\left(\mathrm{Skills}\right) \tridefeq  \exp \left( \sum_i \textrm{Player i Def. Skill} \right)
\]%

%and mention a little more specifically that in order to set the probabilities on the Win_i, we have some probability function with parameters that we will learn, and it takes on the skills of players on court as input (which also are learned)

To understand the relationship between $\left\{Win\right\}$ and $Result$, consider the following example:
...
with solution
\[
\varepsilon = \frac{M_{\mathrm noise}}{M_{\mathrm noise} + M_{\mathrm modelled}}
\]%
where
\begin{align*}
